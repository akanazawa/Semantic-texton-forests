...
%% Divide the data up since it makes too many patches
partition = 4;
for p = 1:partition
    startInd = floor(numTrain/partition*(p-1)+1);
    endInd = floor(numTrain/partition*p);
    data = struct([]);
    dataT = struct([]);
    k = 1; kt = 1;
...
save(sprintf('%s/trainingPatchesAll_%d.mat', DIR.result, p), 'data');
save(sprintf('%s/trainingPatchesAllTransform_%d.mat', DIR.result, p), 'dataT');
end
close(wait);
